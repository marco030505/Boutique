...
import { MOCK_PRODUCTS, CATEGORIES } from '../../mocks/products';
...
export default function Ventas() {
  ...
  }, [search, activeCategory, activeGender]);
  ...
  const handleAddToCart = () => {
    ...
    setCart(prev => {
      const existing = prev.find(i => i.product.id === selectedProduct.id && i.size === selectedSize);
      if (existing) {
        return prev.map(i => 
      ...
      return [...prev, { product: selectedProduct, size: selectedSize, quantity: 1 }];
    });
        ...
        <div className="ventas-toolbar">
          <div className="ventas-search-row">
            <div className="ventas-search-wrapper">
              <span className="ventas-search-icon"><IconSearch /></span>
              <input 
                type="text" 
          ...
          <div className="ventas-filters">
            ...
            <span className="ventas-filter-label" style={{ marginLeft: '12px' }}>Categoría:</span>
            <span 
            ...
            <span className="ventas-product-count">({filteredProducts.length} productos)</span>
          </div>
        </div>
        ...
        <div className="ventas-grid-wrapper">
          {filteredProducts.length > 0 ? (
              ...
              {filteredProducts.map(p => (
                ...
                >
                  <div className="product-card-image-wrap">
                    ...
                    <span className={`product-card-gender-badge product-card-gender-badge--${p.gender}`}>
                      {p.gender}
                    </span>
                    <div className="product-card-add-overlay"><IconPlus /></div>
                  </div>
                  <div className="product-card-info">
                    <span className="product-card-category">{p.category}</span>
                    <span className="product-card-name">{p.name}</span>
                    <span className="product-card-sku">SKU: {p.sku}</span>
            ...
            <div className="ventas-empty">
              <span className="ventas-empty-icon"><IconSearch /></span>
              ...
              <div key={`${item.product.id}-${item.size}`} className="cart-item">
                <div className="cart-item-top">
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                    ...
                    <span className="cart-item-unit-price">${item.product.price.toFixed(2)} c/u</span>
                  </div>
                  <button className="cart-item-remove" onClick={() => removeItem(item.product.id, item.size)}>
                    <IconTrash />
                  </button>
                </div>
                
                <div className="cart-item-bottom">
                  <span className="cart-item-size-badge">Talla: {item.size}</span>
                  <div className="cart-item-qty">
                    <button className="qty-btn" onClick={() => updateQuantity(item.product.id, item.size, -1)}><IconMinus /></button>
                    <span className="qty-value">{item.quantity}</span>
                    <button className="qty-btn" onClick={() => updateQuantity(item.product.id, item.size, 1)}><IconPlus /></button>
                  </div>
                  <span className="cart-item-subtotal">${(item.product.price * item.quantity).toFixed(2)}</span>
                </div>
              </div>
            ))
          ) : (
            <div className="cart-empty">
              <div className="cart-empty-icon"><IconShoppingCart /></div>
              <p>El carrito está vacío. Selecciona productos del catálogo para comenzar la venta.</p>
            </div>
          )}
        </div>
        ...
        <div className="cart-footer">
          <div className="cart-totals">
            ...
            <div className="cart-total-row">
              ...
              <span className="cart-total-amount">${cartSubtotal.toFixed(2)}</span>
            </div>
          </div>
        ...
        <div className="size-modal-overlay" onClick={() => setSelectedProduct(null)}>
          <div className="size-modal" onClick={e => e.stopPropagation()}>
            <div className="size-modal-header">
              <div>
                <h3 className="size-modal-product-name">{selectedProduct.name}</h3>
                <p className="size-modal-price">${selectedProduct.price.toFixed(2)}</p>
              </div>
            ...
            <div className="size-modal-grid">
              {selectedProduct.sizes.map(s => (
                <button
                  key={s}
                  className={`size-btn ${selectedSize === s ? 'size-btn--selected' : ''}`}
      ...
      {showPaymentSuccess && (
        <div className="payment-modal-overlay">
          <div className="payment-modal">
            ...
            <h3>¡Venta exitosa!</h3>
            <div className="payment-modal-total">${cartSubtotal.toFixed(2)}</div>
            <p>El ticket se ha generado correctamente y el inventario ha sido actualizado.</p>
            <button className="payment-modal-close-btn" onClick={closePaymentModal}>
              Nueva Venta
            </button>
          </div>
        </div>
